...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "تعديل ": "إنشاء " ?> مدونة جديدة</h3>
	</div>
	<div class="card-body">
		<form action="" id="blog-form">
            ...
            <div class="form-group">
				<label for="topic_id" class="control-label">الموضوع</label>
            ...
            <div class="form-group">
				<label for="" class="control-label">صورة البنر</label>
            ...
            <div class="form-group">
				<label for="keywords" class="control-label">الكلمات المفتاحية <small>(افصل بين الكلمات باستخدام الفاصلة ",")</small></label>
                <textarea name="keywords" id="" cols="30" rows="3" class="form-control form "><?php echo isset($keywords) ? $keywords : ''; ?></textarea>
			</div>
            <div class="form-group">
				<label for="meta_description" class="control-label">الوصف الوصفي</label>
			...
			<div class="form-group">
                ...
                <option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>منشور</option>
                <option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>غير منشور</option>
                </select>
			</div>
		</form>
	</div>
	<div class="card-footer">
		<button class="btn btn-flat btn-primary" form="blog-form">حفظ</button>
		<a class="btn btn-flat btn-default" href="?page=blogs">إلغاء</a>
	</div>
</div>
...
<script>
    ...
    function upload_images(input,_this){
            ...
            $.ajax({
                ...
                error:err=>{
					console.log(err)
					alert_toast("حدث خطأ أثناء رفع الصور",'error');
					end_loader();
				},
                success:function(resp){
        ...
        function rem_img(_this){
            _conf("هل أنت متأكد من حذف هذه الصورة بشكل دائم؟",'delete_img',["'"+_this.attr('data-path')+"'"])
        }
	$(document).ready(function(){
        
       
        $('.select2').select2({placeholder:"الرجاء الاختيار",width:"relative"})
