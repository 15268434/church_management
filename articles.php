    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                ...
                while($row=$blogs->fetch_assoc()):
                    ?>
                        <a class="list-group-item list-group-item-action my-2 border bg-gradient blog-item" href="<?php echo base_url.$row['blog_url'] ?>">
                            <div class="w-100">
                                <img src="<?php echo validate_image($row['banner_path']) ?>" alt="<?php echo addslashes($row['title']) ?>" align="right" class="bg-dark img-banner-list img-thumbnail m-2">
                                <p class="truncate-5 pt-3"><?php echo strip_tags(stripslashes(html_entity_decode($row['content']))) ?></p><br>
                            </div>
                            <div class="w-100 d-flex justify-content-end">
                                <span class="text-muted">نشر: <?php echo date("M d, Y h:i A", strtotime($row['date_created'])) ?></span><br>
                ...
                <?php if($count_all <= 0): ?>
                    <h4 class="text-center">لا توجد مدونات منشورة لهذا الموضوع حتى الآن.</h4>
                <?php else:  ?>
                <div class="btn-group">
